...
impl GitHubCollector {
    pub fn new(db_url: &str, project_name: &str, owner: &str, repo_name: &str) -> Self {
        Self {
    ...
    pub fn get_releases(self) -> Result<(), reqwest::Error> {
        ...
        let get_url = url.join(url_path.as_str());
        let res: Vec<serde_json::Value> = self.client.get(get_url.unwrap()).send()?.json()?;
        ...
        for release in res.iter() {
            ...
            //let pub_date = rel["published_at"].as_str().unwrap();
